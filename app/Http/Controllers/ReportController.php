...
class ReportController extends Controller
{
    ...
    public function general_donation_report(){
        ...
        $date1=date('Y-m-d',strtotime(substr($daterange,0,10)));
        $date2=date('Y-m-d',strtotime(substr($daterange,13)));
        $donation=DB::SELECT("SELECT * FROM `GeneralDonation` join add_members on add_members.p_id=GeneralDonation.member_id where date BETWEEN '$date1' and '$date2'");
        return view('Reports.general_donation_report',['donation'=>$donation,'daterange'=>$daterange]);
    }
